          ...
          <thead>
            <tr>
              ...
              {titles?.map((title, idx) => (
                <th key={idx} className={`${dataFormat ? dataFormat[idx] : ""} ${dataSize ? dataSize[idx] : ""}`}>
                  {title}
                </th>
              ))}
                ...
                <Fragment key={index}>
                  ...
                  >
                    ...
                    {item.data?.map((d: any, idx: number) => (
                      <td className={`${dataFormat ? dataFormat[idx] : ""} ${dataSize ? dataSize[idx] : ""}`} key={idx}>
                    ...
                    {actions && actions?.length > 0 ? (
                      <td className="actions task" onClick={(e) => e.stopPropagation()}>
                        {actions.map((el, idx) => (
                          <div
                            className={`action-btn ${el.disable ? "action-item-disable" : ""}`}
                            style={{ color: el.color }}
                            onClick={() => (el.disable ? null : el.callback(item))}
                            key={idx}
                          >
                            {el.icon}
                            <span>{el.title}</span>
